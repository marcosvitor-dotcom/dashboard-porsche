...
import { Calendar, Filter, ChevronUp, ChevronDown, ChevronsUpDown } from "lucide-react"
...
const CriativosPinterest: React.FC = () => {
  ...
  const [sortKey, setSortKey] = useState<"pontuacao" | "impressions" | "cost" | "clicks" | "ctr">("pontuacao")
  ...
  const SortIcon: React.FC<{ col: typeof sortKey }> = ({ col }) => {
    ...
    return sortOrder === "desc" ? <ChevronDown className="w-3 h-3 ml-1" /> : <ChevronUp className="w-3 h-3 ml-1" />
  }
  ...
  const filteredData = useMemo(() => {
    ...
    finalData.sort((a, b) => {
      let va = 0, vb = 0
      if (sortKey === "pontuacao") {
        va = a.pontuacaoCriativo ?? -1
        vb = b.pontuacaoCriativo ?? -1
      } else if (sortKey === "impressions") { va = a.impressions; vb = b.impressions }
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-red-600 text-white">
                ...
                <th className="text-right py-3 px-4 font-semibold min-w-[7.5rem] cursor-pointer select-none hover:bg-red-700 transition-colors" onClick={() => handleSort("cost")}>
                  <div className="flex items-center justify-end">Investimento<SortIcon col="cost"/></div>
                </th>
                <th className="text-right py-3 px-4 font-semibold min-w-[7.5rem] cursor-pointer select-none hover:bg-red-700 transition-colors" onClick={() => handleSort("impressions")}>
                  <div className="flex items-center justify-end">Impressões<SortIcon col="impressions"/></div>
                </th>
                <th className="text-right py-3 px-4 font-semibold min-w-[7.5rem] cursor-pointer select-none hover:bg-red-700 transition-colors" onClick={() => handleSort("clicks")}>
                  <div className="flex items-center justify-end">Cliques<SortIcon col="clicks"/></div>
                </th>
                <th className="text-right py-3 px-4 font-semibold min-w-[7.5rem] cursor-pointer select-none hover:bg-red-700 transition-colors" onClick={() => handleSort("ctr")}>
                ...
                <th className="text-right py-3 px-4 font-semibold min-w-[7.5rem] cursor-pointer select-none hover:bg-red-700 transition-colors" onClick={() => handleSort("pontuacao")}>
                  <div className="flex items-center justify-end">Pontuação<SortIcon col="pontuacao"/></div>
                </th>
              </tr>
            </thead>
            <tbody>
