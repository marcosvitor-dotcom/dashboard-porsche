...
interface CreativeModalLinkedInProps {
...
const CreativeModalLinkedIn: React.FC<CreativeModalLinkedInProps> = ({ creative, isOpen, onClose }) => {
  if (!isOpen || !creative) return null

  const ctr = creative.impressions > 0 ? (creative.clicks / creative.impressions) * 100 : 0
  const vtr = creative.videoViews > 0 ? (creative.videoCompletions / creative.videoViews) * 100 : 0
      ...
      <div className="bg-white rounded-lg max-w-5xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b">
          <div className="flex-1">
            ...
            {creative.campaignGroupName && (
              <p className="text-xs text-gray-400 mt-0.5">{creative.campaignGroupName}</p>
            )}
          </div>
            ...
            <div className="space-y-4">
              ...
              >
                {creative.creativeThumbnail ? (
                  <img
                    src={creative.creativeThumbnail}
                    alt="Criativo LinkedIn"
                    className="w-full h-full object-contain"
                    onError={(e) => {
                      const target = e.target as HTMLImageElement
                      target.style.display = "none"
                      if (target.parentElement) {
                        target.parentElement.innerHTML = '<div class="flex flex-col items-center gap-2 text-gray-400"><span class="text-sm">Sem imagem</span></div>'
                      }
                    }}
                  />
                ) : (
                  <div className="flex flex-col items-center gap-3 text-gray-400">
                    ...
                    <div className="text-center">
                      <div className="text-sm font-medium">LinkedIn Ads</div>
                ...
                <div className="space-y-2 text-sm">
                  ...
                  {creative.campaignGroupName && (
                    <div className="flex justify-between">
                      ...
                      <span className="text-gray-900 text-right max-w-[200px] truncate">{creative.campaignGroupName}</span>
                    </div>
                  )}
                  {creative.creativeText && (
                    <div className="mt-2">
                      <span className="text-gray-600">Texto do anúncio:</span>
                      <p className="text-gray-900 text-xs mt-1 italic">{creative.creativeText}</p>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-purple-50 rounded-lg p-4 text-center">
                  <div className="text-2xl font-bold text-purple-600">{formatNumber(creative.totalEngagements)}</div>
...
export default CreativeModalLinkedIn
